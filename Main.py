...
def ChatLogIntegration():
    ...
    for entry in json_data:
        if entry["role"] == "user":
            formatted_chatalog += f"User: {entry['content']}\n"
        elif entry["role"] == "assistant":  
            formatted_chatalog += f"Assistant: {entry['content']}\n"
    formatted_chatalog = formatted_chatalog.replace("User", Username + " ")
    formatted_chatalog = formatted_chatalog.replace("Assistant", Assistantname + " ")

    with open(TempDirectoryPath('Database.data'), 'w', encoding='utf-8') as file:
        file.write(AnswerModifier(formatted_chatalog))
